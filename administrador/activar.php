...
if($id=='' || $token==''){
  ...
  if($token==$token_tmp){
    ...
    if($sql->fetchColumn() > 0){
        ...
        $borrar=$con->prepare("UPDATE ciudadoresguar  SET situacion='1' WHERE cod_empresa='$id'");
        $borrar->execute();

        if($borrar){
            echo'<script>alert("La empresa ha sido activada desde este momento cualquier persona la podra visualizar");
            window.location="../administrador/bloqueadas.php";
            </script>';
        }
