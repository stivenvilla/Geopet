...
if(!isset($sesion)){
  ...
  //$busquedad=$_POST['finalizarReservas'];
  $db=new database();
  $con = $db->conectar();
  $estado='activo';
  $sql=$con->prepare("SELECT tblreserva.cod_reserva, tblreserva.cod_empresa, tblreserva.doc_id, tblmascota.nombreMacota as mascota, tblservicio.nombre as servicio,tblreserva.fechaAsignada, tblreserva.estado FROM tblreserva INNER JOIN tblmascota ON tblreserva.cod_mascota=tblmascota.cod_mascota INNER JOIN tblservicio ON tblreserva.servicio= tblservicio.cod_servicio WHERE tblreserva.cod_empresa='$sesion' AND estado='$estado'");
  $sql->execute();
  $resultado=$sql->fetchAll(PDO::FETCH_ASSOC);

  if(!$resultado){
   echo '
    <script>alert("No se han encontrado reservas activas"); 
...
<html lang="en" dir="ltr">
  ...
  <input type="checkbox" id="check">
    ...
    <div class="sidebar">
      ...
      <a href="../empresa/finalizarReservas.php" class="active">
      <i class="fas fa-calendar"></i>
        <span>Finalizar</span>
      </a>
  ...
  <!-- este codigo lo que realiza es mostrar la informacion de la base de datos 
pero ademas lo que hara es que la empresa puede darle al boton de finalizar esto lo que 
hara es dar por hecho terminada la reservacion -->
    <div id="main-container">
      ...
      <?php foreach($resultado as $row ){ ?>
			<tr>
                ...
                <th><a href="acabarReservacion.php?id=<?php echo $row['cod_reserva'];?>&token=<?php echo hash_hmac('sha1',$row['cod_reserva'], KEY_TOKEN);?>">Finalizar</a></th>
